...
def trackMultipleObjects():
    ...
    while True:
        ...
        if len(classIds) != 0:
            for i in indices:
                i = i
                if 0 <= classIds[i] - 1 < len(classNames):
                    box = bbox[i]
                    ...
                    cv2.putText(image, classNames[classIds[i] - 1] + " " + confidence, (x + 10, y + 20),
                                font, 1, color, 2)
        ...
        for carID in carTracker.keys():
            ...
            if carID not in startTracker and mark2 > ty+th > mark1 and ty < mark1:
                ...
                if speed > speedLimit:
                    print('ID : {} : {} kmph - OVERSPEED'.format(carID, speed))
                    saveCar(speed,image[ty:ty+th, tx:tx+tw])
                else:
                    print('ID : {} : {} kmph'.format(carID, speed))
